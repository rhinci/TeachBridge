...
import CreateSectionModal from '../components/CreateSectionModal'; // Импортируем модальное окно
...
const StudyChatPage = () => { 
  ...
  // Состояние для модального окна создания раздела
  ...
  const loadSections = async () => {
    try {
      ...
      // Можно показать уведомление, но не блокировать загрузку чата
    }
  };
  ...
  // Функция для создания нового раздела
  const handleCreateSection = async (sectionData) => {
    try {
      setCreatingSection(true);
      
      // Создаем раздел через API
      const newSection = await chatService.createChatSection(chatId, sectionData);
      ...
      alert(`Раздел #${newSection.name} успешно создан!`);
  ...
  // Функция для удаления раздела (дополнительно)
  const handleDeleteSection = async (sectionId, sectionName) => {
    if (!sectionId || sectionName === '#general') {
      alert('Нельзя удалить основной раздел #general');
      return;
    }

    if (!window.confirm(`Вы уверены, что хотите удалить раздел #${sectionName}?`)) {
      return;
    }

    try {
      await chatService.deleteChatSection(sectionId);
      
      // Обновляем список разделов
      await loadSections();
      
      // Если удалили активный раздел, переключаемся на #general
      if (activeSection?.id === sectionId) {
        const generalSection = sections.find(s => s.name === '#general');
        setActiveSection(generalSection || sections[0]);
      }
  ...
  // Разрешаем создавать разделы: преподавателям, директорам и администраторам
  const canUserCreateSections = ['teacher', 'director', 'admin'].includes(userRole);
  
  // Дополнительно: проверяем, что пользователь имеет доступ к этому чату
  // (это уже проверяется в API, но для UI можно добавить)
  ...
  // (для отображения дополнительной информации или скрытия кнопки если нет доступа)
  const isUserInChat = () => {
    if (!chat || !participants.length) return false;
                ...
                <div className="study-sections-list">
                  {sections.map(section => (
                    ...
                    >
                      ...
                      {/* Кнопка удаления раздела (только для создателей и преподавателей, кроме #general) */}
                      {canUserCreateSections && section.name !== '#general' && (
                        <button 
                          className="study-delete-section-btn"
                          onClick={(e) => {
                            e.stopPropagation();
